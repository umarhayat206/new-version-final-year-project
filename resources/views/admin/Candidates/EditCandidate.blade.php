...
<div class="card card-info card-outline">
    ...
    <div class="card-body">
        ...
        <form action="{{route('candidates.store')}}" method="POST" enctype="multipart/form-data">
            ...
            <label>Name</label>
            <input type="text" class="form-control form-control-lg" placeholder="Enter Candidate Name" name="name" value="{{$user->name}}"
            </div>
            <div class="form-group">
            <label>Candidate CNIC</label>
            <input type="text"  data-inputmask="'mask': '99999-9999999-9'"  placeholder="XXXXX-XXXXXXX-X"  name="cnic" class="form-control" value="{{$user->cnic}}">
            </div>
            <div class="form-group">
        ...
        <select class="select2 form-control" name="areas[]" multiple="multiple" data-placeholder="Select a State" style="width: 100%;">
          ...
          <option value="{{$area->id}}"{{$area->id==$userArea?'selected':''}}>{{$area->name}}</option>
          @endforeach
          @endforeach
        </select>
        </div>
            <div class="form-group">
              ...
              <select class="form-control" name="party">
                ...
                <option value="{{$party->id}}"{{$party->id==$user->candidateParty->party_id?'selected':''}}>{{$party->name}}</option>
                @endforeach
            </select>
            </div>
            <div class="form-group">
            <label>Candidate Adress</label>
            <input  type="address" class="form-control" name="address" placeholder="Enter Adreess" value="{{$user->address}}">
            </div>
            <div class="form-group">
